...
const NavButton = ({ href = '#', label = 'Button', asButton = false, onClick = undefined }) => {
  const letters = Array.from(label)
  const letters2 = Array.from(label)
  ...
  const content = (
    <>
      ...
      <div className="txt-wrapper">
        <div className="txt-1">
          {letters.map((ch, i) => (
            <span key={`a-${i}`} className="btn-letter">{ch}</span>
          ))}
        </div>
        <div className="txt-2">
          {letters2.map((ch, i) => (
            <span key={`b-${i}`} className="btn-letter">{ch}</span>
          ))}
        </div>
      </div>
    </>
  ...
  return (
    <StyledWrapper>
      <div className="btn-wrapper">
        {asButton ? (
          <button type="button" className="btn" aria-label={label} onClick={onClick}>
            {content}
          </button>
        ) : isExternal ? (
          <a href={href} className="btn" aria-label={label} target="_blank" rel="noopener noreferrer">
            {content}
          </a>
        ) : (
          <Link href={href} legacyBehavior>
            <a className="btn" aria-label={label}>{content}</a>
          </Link>
        )}
      </div>
    </StyledWrapper>
...
const StyledWrapper = styled.div`
  ...
  .btn:active::before { box-shadow: 0 -8px 12px -6px #fffa inset, 0 -16px 16px -8px hsla(var(--highlight-color-hue), 100%, 70%, 80%) inset, 1px 1px 1px #fff4, 2px 2px 2px #fff2, -1px -1px 1px #0002, -2px -2px 2px #0001; }
